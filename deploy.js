...
zipFolder(folderName, zipName, async function(err) {
  if (err) {
    ...
    try {
      if (allowUpload) {
        ...
        if (allowPublish && upload.uploadState === 'SUCCESS') {
          ...
          console.log('Not going to publish. Either not allowed or upload failed');
        }
      }
    } catch (e) {
      console.log(e);
    ...
    // Increase version number AFTER zipping
    ...
    fs.writeFile(manifestFileName, JSON.stringify(manifest, null, 2), function(
      err
    ) {
