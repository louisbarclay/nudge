...
function execSettings(settings) {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    if (currentTab.url.startsWith('chrome-extension://') && currentTab.url.includes('html/pages/off')) {
      ...
      Object.keys(settings.domains).forEach(function (nudgeDomain) {
        if (extractDomain(url).includes(nudgeDomain)) {
          ...
          if (!settings.domains[nudgeDomain].nudge) {
            ...
            console.log(`You're on ${nudgeDomain} but you're not nudging it at the moment`)
          } else if (!domain) {
    ...
    if (!isInDomainList) {
      ...
      newDomain.substring(0, 4) === 'www.' && (newDomain = newDomain.substring(4));
    }
    ...
    chrome.runtime.sendMessage({ type: "get_localStorage" }, function (response) {
      ...
      function setToSnooze() {
        function updateButtonText() {
          button.innerHTML = `Stop snooze (${logMinutes(Math.round(moment.unix(snoozeEnd / 1000).diff(moment()) / 1000))})`;
        };
        updateButtonText();
      ...
      if (domain) {
        if (domain in today) {
          timeToday.innerHTML = logMinutesNoSeconds(today[domain].runningCounter + ('time' in today[domain] ? today[domain].time / 1000 : 0)) + ' ';
        } else {
        ...
        if (whitelist) {
          el('dont-nudge').innerHTML = `Change your whitelist to nudge this site`
        ...
        el('dont-nudge').onclick = function () {
          if (domain) {
            ...
            if (isInDomainList) {
              ...
              console.log(newDomain);
              console.log("Not in domain list, adding fresh");
              domain = newDomain;
      ...
      el('reset-hide').onclick = function () {
        resetPage(settings.divs, domain)
      }
    });
  });
}

// Function to reset hidden divs
function resetPage(divs, domain) {
